...
export const tripsDeleteCommand = defineCommand({
    meta: { name: 'delete', description: 'Permanently delete a trip.' },
    args: {
        ...
        try {
            if (!args.yes) {
                const ok = await confirm({
                    message: `Permanently delete trip ${args.tripId}? This cannot be undone.`,
                    default: false,
                })
                if (!ok) {
            ...
            const { data, error } = await client.DELETE('/v1/trips/{id}', {
            ...
            if (args.json) {
                ...
                process.stdout.write(`${colors.green('✓')} Deleted trip ${args.tripId}.\n`)
            }
        } catch (err) {
            reportAndExit(err)
        }
