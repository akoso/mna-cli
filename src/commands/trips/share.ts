...
export const tripsShareCommand = defineCommand({
    meta: { name: 'share', description: 'Generate a shareable link for a trip.' },
    args: {
    ...
    async run({ args }) {
        try {
            ...
            const { data, error } = await client.POST('/v1/trips/{id}/share-link', {
            ...
            if (args.json) {
                ...
                process.stdout.write(`${colors.green('✓')} ${data!.url}\n`)
            }
        } catch (err) {
            reportAndExit(err)
        }
